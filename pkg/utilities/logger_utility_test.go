...
import (
	"github.com/ereb-or-od/kenobi-logger/pkg/enumeration"
	"go.uber.org/zap"
...
func TestToZapFieldShouldReturnIntWhenValueEqualsToInteger(t *testing.T) {
	...
	if zapField == nil {
		t.Error("zapField could not be nil")
	}
...
func TestToZapFieldShouldReturnErrorWhenKeyIsEmpty(t *testing.T) {
	zapField, err := toZapField("", true)
	if err == nil {
		t.Error("zapField must have an error when key is nil")
	}
	if zapField != nil {
		t.Error("zapField must be nil when key is nil")
	}
}
